...
def load_files_from_upload(uploaded_files) -> tuple[dict, list]:
    """Read bytes from uploaded files, expanding ZIPs automatically."""
    file_store = {}
    file_names = []
            ...
            try:
                with zipfile.ZipFile(io.BytesIO(raw)) as zf:
                    for member in zf.namelist():
                        ...
                        if member_name not in file_store:
                            file_store[member_name] = member_bytes
                            file_names.append(member_name)
            except Exception as e:
                st.warning(f"⚠️ Could not read ZIP `{name}`: {e}")
        else:
            if name not in file_store:
                file_store[name] = raw
                file_names.append(name)

    return file_store, file_names
...
for key, default in {
    ...
    "file_store": {},
    "file_names": [],
    # Track which sub-steps have been run
...
with st.sidebar:
    ...
    st.markdown("""
- **Extracts** text from PDF, DOCX, TXT, ZIP
...
""")
    ...
    st.markdown("`.pdf` · `.docx` · `.txt` · `.zip` (containing the above)")

    if st.session_state.step > 0 or st.session_state.file_names:
        st.markdown("---")
        if st.button("🔄 Start New Review", use_container_width=True):
            for k in ["step","abstractions","themes","synthesis","file_store","file_names",
                      "abstraction_done","themes_done","synthesis_done"]:
                if k == "file_store":
                    st.session_state[k] = {}
                elif k in ("abstractions","file_names"):
                    st.session_state[k] = []
                elif k in ("themes","synthesis","api_key"):
                    st.session_state[k] = ""
                elif k == "step":
...
if st.session_state.step == 0:
    ...
    st.caption("Select one or more PDF, DOCX, or TXT files — or a ZIP containing them. You can upload as many as you need.")
    ...
    # ✅ FIX: Read and persist bytes into session_state immediately on upload
    if uploaded:
        file_store, file_names = load_files_from_upload(uploaded)
        # Persist into session state right away so they survive reruns
        st.session_state.file_store = file_store
        st.session_state.file_names = file_names

    # Show upload confirmation from session state (persists across reruns)
    if st.session_state.file_names:
        file_store = st.session_state.file_store
        file_names = st.session_state.file_names

        # ✅ FIX: Success message now reads from session state, not local vars
        st.markdown(
            f'<div class="upload-success-box">'
            f'✅ <b>{len(file_names)} file(s) loaded and ready to analyse</b>'
            f'</div>',
            unsafe_allow_html=True
        )
        ...
        # ✅ FIX: Button reads from session_state.file_names, not local vars
        if not st.session_state.api_key:
            st.warning("⚠️ Enter your OpenAI API key in the sidebar first")
        else:
            st.markdown(
                '<div class="action-hint">👇 Click <b>Analyse Documents</b> to extract structured data from each paper using GPT-4o-mini.</div>',
                unsafe_allow_html=True
            )
            if st.button("🚀 Analyse Documents", type="primary", use_container_width=False):
                st.session_state.step = 1
                st.session_state.abstraction_done = False
                st.rerun()
    else:
        ...
        <div style="border:2px dashed #cbd5e1;border-radius:12px;padding:3rem;text-align:center;color:#94a3b8;margin-top:1rem">
            ...
            <div style="font-size:0.85rem;margin-top:.25rem">PDF · DOCX · TXT · ZIP (batch) · Multiple files supported</div>
        </div>
        """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not file_names:
        ...
        if st.button("← Back to Upload"):
            ...
            st.rerun()
    elif not st.session_state.abstraction_done:
        # ✅ FIX: Show the button FIRST — user must click to start abstraction
        st.markdown(
            '<div class="action-hint">👇 Click <b>Do Data Abstraction</b> to extract metadata (authors, year, methods, findings, contributions, limitations, keywords) from all papers.</div>',
            unsafe_allow_html=True
        )
        col_btn, _ = st.columns([1, 3])
        with col_btn:
            run_abstraction = st.button("🔍 Do Data Abstraction", type="primary", use_container_width=True)

        if run_abstraction:
            st.info(f"Extracting structured metadata from {len(file_names)} paper(s) using GPT-4o-mini…")
            ...
            st.session_state.abstraction_done = True
            status_box.success(f"✅ Successfully abstracted {len(abstractions)} paper(s)!")
            time.sleep(0.8)
            st.rerun()
    else:
        # Abstraction done — show summary table and next action button
        abstractions = st.session_state.abstractions
        st.success(f"✅ Data abstraction complete — {len(abstractions)} paper(s) processed.")
        ...
        st.markdown(
            '<div class="action-hint">👇 Click <b>Find Key Themes</b> to identify cross-cutting themes across all papers and generate an academic synthesis.</div>',
            unsafe_allow_html=True
        )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not st.session_state.themes_done:
        with st.spinner("🧵 Identifying cross-cutting themes across all papers…"):
            try:
    ...
    if st.session_state.themes_done and not st.session_state.synthesis_done:
        with st.spinner("✍️ Writing formal academic synthesis / discussion…"):
            try:
                synthesis = write_synthesis_report(abstractions, st.session_state.themes)
    ...
    if st.session_state.themes_done and st.session_state.synthesis_done:
        st.success("✅ Theme identification and synthesis complete!")
        st.markdown(
            '<div class="action-hint">👇 Click <b>View Full Results</b> to see the complete analysis, all paper abstractions, summary table, and download your export package.</div>',
            unsafe_allow_html=True
        )
        col1, col2 = st.columns([1, 3])
        with col1:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Tab 2
    with tab2:
        sub1, sub2 = st.tabs(["🧵 Key Themes", "✍️ Discussion / Synthesis Narrative"])
        with sub1:
            st.markdown(themes)
        with sub2:
    ...
    # ── Tab 3
    with tab3:
        st.markdown("#### Corpus Overview — Summary Table for All Papers")
        import pandas as pd
        rows = []
        for p in abstractions:
            title = p.get("title","")
    ...
    with col2:
        if st.button("← Back to Abstraction", use_container_width=True):
            st.session_state.step = 1
            st.rerun()
    with col3:
        st.caption(
