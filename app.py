...
def _landing_html(error=False):
    err = "block" if error else "none"
...
html,body{{background:#05070d;font-family:'JetBrains Mono',monospace;min-height:100%;overflow-x:hidden}}
...
.g1{{width:500px;height:500px;top:-180px;left:50%;transform:translateX(-50%);background:radial-gradient(circle,rgba(99,102,241,0.14) 0%,transparent 70%)}}
.g2{{width:320px;height:320px;top:30%;right:-100px;background:radial-gradient(circle,rgba(236,72,153,0.08) 0%,transparent 70%)}}
.g3{{width:280px;height:280px;top:40%;left:-80px;background:radial-gradient(circle,rgba(6,182,212,0.07) 0%,transparent 70%)}}
.gf{{position:fixed;bottom:0;left:0;right:0;height:180px;pointer-events:none;z-index:0;
  background:linear-gradient(to top,rgba(5,7,13,1) 0%,transparent 100%),
...
.page{{position:relative;z-index:1;display:flex;flex-direction:column;align-items:center;width:100%;padding-bottom:3rem}}
.strip{{width:100%;display:flex;justify-content:space-between;align-items:center;padding:.75rem 1.5rem;border-bottom:1px solid rgba(255,255,255,0.03)}}
...
.mascot{{width:100%;max-width:700px;padding:0 1rem;margin-top:.5rem}}
.mascot svg{{width:100%;height:auto;overflow:visible;filter:drop-shadow(0 0 40px rgba(99,102,241,.22)) drop-shadow(0 0 80px rgba(236,72,153,.09))}}
.title{{text-align:center;padding:0 1rem;margin-top:-.5rem}}
.title h1{{font-size:clamp(2rem,8vw,3.5rem);font-weight:800;color:#f1f5f9;letter-spacing:-.02em;line-height:1;margin-bottom:.3rem}}
...
.pill{{background:rgba(255,255,255,.025);border:1px solid rgba(255,255,255,.06);border-radius:2px;padding:.25rem .75rem;font-size:.52rem;letter-spacing:.1em;text-transform:uppercase;color:rgba(148,163,184,.38)}}
.pill.on{{background:rgba(99,102,241,.1);border-color:rgba(99,102,241,.3);color:rgba(165,180,252,.85)}}
.hr{{width:min(340px,90%);height:1px;margin:.2rem 0 1rem;background:linear-gradient(90deg,transparent,rgba(99,102,241,.4),rgba(165,180,252,.5),rgba(99,102,241,.4),transparent)}}
.eyebrow{{font-size:.52rem;letter-spacing:.26em;text-transform:uppercase;color:rgba(165,180,252,.35);margin-bottom:.85rem;text-align:center}}
.form{{width:min(340px,90%);display:flex;flex-direction:column;gap:.55rem}}
.iw{{position:relative}}
.iw::before{{content:'';position:absolute;left:0;top:15%;height:70%;width:1px;background:rgba(99,102,241,.55)}}
input[type=password]{{
  width:100%;padding:.65rem 1rem .65rem 1.3rem;
  background:rgba(10,12,24,.9);border:1px solid rgba(99,102,241,.22);border-radius:3px;
  color:#e2e8f0;font-family:'JetBrains Mono',monospace;font-size:.78rem;letter-spacing:.06em;
...
input[type=password]:focus{{border-color:rgba(99,102,241,.55);box-shadow:0 0 0 3px rgba(99,102,241,.08)}}
button[type=submit]{{
  ...
  box-shadow:0 0 18px rgba(99,102,241,.22),0 0 0 1px rgba(99,102,241,.28);transition:all .2s}}
button[type=submit]:active{{transform:scale(.98)}}
.err{{display:{err};font-size:.6rem;letter-spacing:.1em;color:rgba(248,113,113,.9);text-align:center}}
.hr2{{width:min(340px,90%);height:1px;margin-top:.8rem;background:linear-gradient(90deg,transparent,rgba(99,102,241,.15),transparent)}}
</style>
</head>
<body>
<div class="g g1"></div><div class="g g2"></div><div class="g g3"></div><div class="gf"></div>
<div class="page">
<div class="strip"><span class="sl">Agent43 &middot; System v2.0</span><span class="sr">AI &middot; Academic &middot; Engine</span></div>

<div class="mascot"><svg viewBox="0 0 900 500" xmlns="http://www.w3.org/2000/svg">
<defs>
...
<ellipse cx="93" cy="54" rx="28" ry="20" fill="url(#spb)" opacity="0.72"/><ellipse cx="76" cy="40" rx="18" ry="13" fill="rgba(192,38,211,0.5)"/><ellipse cx="113" cy="42" rx="15" ry="10" fill="rgba(124,58,237,0.5)"/>
...
<circle cx="384" cy="34" r="9" fill="#312e81" stroke="#818cf8" stroke-width="1.5" filter="url(#gv)"/><circle cx="384" cy="34" r="5" fill="#a78bfa" filter="url(#gv)"/>
<rect x="512" y="40" width="8" height="28" rx="4" fill="#4f46e5" stroke="#818cf8" stroke-width="1"/>
<circle cx="516" cy="34" r="9" fill="#312e81" stroke="#818cf8" stroke-width="1.5" filter="url(#gv)"/><circle cx="516" cy="34" r="5" fill="#a78bfa" filter="url(#gv)"/>
...
<circle cx="407" cy="119" r="14" fill="#0891b2" filter="url(#gc)"/><circle cx="407" cy="119" r="9" fill="#06b6d4"/><circle cx="407" cy="119" r="5" fill="#a5f3fc"/>
...
<circle cx="493" cy="119" r="14" fill="#a21caf" filter="url(#gp)"/><circle cx="493" cy="119" r="9" fill="#e879f9"/><circle cx="493" cy="119" r="5" fill="#f0abfc"/>
...
<!-- FORM: submits directly to parent page URL via target="_top" -->
<form class="form" method="GET" action="" target="_top">
  <div class="iw">
    <input type="password" name="_a43" id="pwd" placeholder="Enter access password" autocomplete="current-password"/>
  </div>
  <button type="submit">Enter System &nbsp;&#x2192;</button>
  <div class="err">Incorrect password &mdash; try again.</div>
</form>
<div class="hr2"></div>
</div>
</body>
...
def auth_gate():
    ...
    # Read password submitted via form GET
    ...
    if pwd:
        ...
        if pwd == APP_PASSWORD:
            ...
            # Wrong password — re-render with error visible
            components.html(_landing_html(error=True), height=1100, scrolling=True)
            return False

    components.html(_landing_html(error=False), height=1100, scrolling=True)
    return False
