...
def auth_gate():
    ...
    pwd = st.query_params.get("_a43", "")
    if pwd:
        st.query_params.clear()
        if pwd == APP_PASSWORD:
            st.session_state["authenticated"] = True
            st.rerun()
        # Wrong password — fall through and show error

    show_error = bool(pwd)  # True if wrong password was just submitted

    err_display = "flex" if show_error else "none"
    ...
    /* Make stApp fill viewport with our background */
    .stApp {{
        background: #05070d !important;
        min-height: 100vh;
    }}
    /* Hide the default block container padding */
    .block-container {{
        padding: 0 !important;
    ...
    .lp-g1 {{ width:500px; height:500px; top:-180px; left:50%; transform:translateX(-50%);
        background: radial-gradient(circle, rgba(99,102,241,0.13) 0%, transparent 70%); }}
    .lp-g2 {{ width:320px; height:320px; top:30%; right:-100px;
        background: radial-gradient(circle, rgba(236,72,153,0.08) 0%, transparent 70%); }}
    .lp-g3 {{ width:280px; height:280px; top:40%; left:-80px;
    ...
    /* inner content above fixed elements */
    .lp-inner {{ position: relative; z-index: 1; width: 100%; display: flex; flex-direction: column; align-items: center; }}
    /* strip */
    .lp-strip {{
    ...
    .lp-sl {{ font-size:.55rem; letter-spacing:.2em; text-transform:uppercase; color:rgba(99,102,241,.6); white-space:nowrap; }}
    .lp-sr {{ font-size:.55rem; letter-spacing:.2em; text-transform:uppercase; color:rgba(255,255,255,.1); white-space:nowrap; }}
    /* mascot */
    .lp-mascot {{
        width:100%; max-width:700px; padding:0 1rem; margin-top:.5rem;
    }}
    .lp-mascot svg {{
    ...
    /* title */
    .lp-title {{ text-align:center; padding:0 1rem; margin-top:-.5rem; }}
    .lp-title h1 {{
    ...
    .lp-tag {{ font-size:.55rem; letter-spacing:.22em; color:rgba(148,163,184,.38); text-transform:uppercase; margin-bottom:1.1rem; font-family:'JetBrains Mono',monospace; }}
    /* pills */
    .lp-pills {{ display:flex; gap:.4rem; justify-content:center; flex-wrap:wrap; padding:0 1rem .8rem; }}
    .lp-pill {{
    ...
    /* form */
    .lp-form {{ width:min(340px,90%); display:flex; flex-direction:column; gap:.55rem; }}
    ...
    .lp-form input[type=password]::placeholder {{ color:rgba(71,85,105,.6); letter-spacing:.1em; }}
    ...
    .lp-form button[type=submit]:hover {{
        box-shadow:0 4px 20px rgba(99,102,241,.35), 0 0 0 1px rgba(165,180,252,.3);
        background:linear-gradient(135deg,#4f46e5 0%,#7c3aed 100%);
    }}
    .lp-form button[type=submit]:active {{ transform:scale(.98); }}
    .lp-err {{
        display:{err_display};
        align-items:center; justify-content:center;
        font-size:.6rem; letter-spacing:.1em;
        color:rgba(248,113,113,.9); text-align:center;
        font-family:'JetBrains Mono',monospace;
        gap:.4rem;
    }}
    </style>
    ...
    <div class="lp-wrap">
      ...
      <div class="lp-inner">
        ...
        <defs>
          <filter id="lp-gv" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="5" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
          <filter id="lp-gc" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="4" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
          <filter id="lp-gp" x="-60%" y="-60%" width="220%" height="220%"><feGaussianBlur stdDeviation="9" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
          <filter id="lp-gs" x="-80%" y="-80%" width="260%" height="260%"><feGaussianBlur stdDeviation="20"/></filter>
          <linearGradient id="lp-bg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#1e1b4b"/><stop offset="100%" stop-color="#0f0c24"/></linearGradient>
          <linearGradient id="lp-hg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#2d2a5e"/><stop offset="100%" stop-color="#1a1740"/></linearGradient>
          <linearGradient id="lp-sg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#0e7490"/><stop offset="100%" stop-color="#0c4a6e"/></linearGradient>
          <linearGradient id="lp-alg" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stop-color="#312e81"/><stop offset="100%" stop-color="#1e1b4b"/></linearGradient>
          <linearGradient id="lp-arg" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stop-color="#1e1b4b"/><stop offset="100%" stop-color="#312e81"/></linearGradient>
          <linearGradient id="lp-spg" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="#c026d3"/><stop offset="100%" stop-color="#7c3aed"/></linearGradient>
          <radialGradient id="lp-spb" cx="30%" cy="30%" r="70%"><stop offset="0%" stop-color="#f0abfc" stop-opacity="0.9"/><stop offset="50%" stop-color="#c026d3" stop-opacity="0.5"/><stop offset="100%" stop-color="#7c3aed" stop-opacity="0"/></radialGradient>
          <radialGradient id="lp-elg" cx="50%" cy="50%" r="50%"><stop offset="0%" stop-color="#a5f3fc"/><stop offset="60%" stop-color="#06b6d4" stop-opacity="0.8"/><stop offset="100%" stop-color="#0891b2" stop-opacity="0"/></radialGradient>
          <radialGradient id="lp-erg" cx="50%" cy="50%" r="50%"><stop offset="0%" stop-color="#f0abfc"/><stop offset="60%" stop-color="#e879f9" stop-opacity="0.8"/><stop offset="100%" stop-color="#a21caf" stop-opacity="0"/></radialGradient>
        </defs>
        <!-- BIG 43 WALL TAG -->
        <g opacity="0.15" filter="url(#lp-gp)"><text x="100" y="420" font-family="Arial Black,sans-serif" font-size="310" font-weight="900" fill="none" stroke="#c026d3" stroke-width="5" stroke-linejoin="round">43</text></g>
        ...
        <!-- SPLATTER -->
        <g opacity="0.22" filter="url(#lp-gv)"><circle cx="168" cy="148" r="4" fill="#a78bfa"/><circle cx="153" cy="166" r="2.5" fill="#c4b5fd"/><circle cx="184" cy="160" r="3" fill="#818cf8"/><circle cx="143" cy="183" r="2" fill="#a78bfa"/><circle cx="198" cy="143" r="2" fill="#c4b5fd"/></g>
        <!-- SHADOW -->
        <ellipse cx="450" cy="492" rx="220" ry="16" fill="rgba(99,102,241,0.10)" filter="url(#lp-gs)"/>
        <!-- LEGS -->
        <rect x="368" y="418" width="58" height="65" rx="6" fill="url(#lp-bg)" stroke="#4f46e5" stroke-width="1.5"/>
        ...
        <rect x="474" y="418" width="58" height="65" rx="6" fill="url(#lp-bg)" stroke="#4f46e5" stroke-width="1.5"/>
        ...
        <!-- TORSO -->
        <rect x="330" y="238" width="240" height="190" rx="18" fill="url(#lp-bg)" stroke="#4f46e5" stroke-width="2"/>
        <rect x="340" y="250" width="220" height="5" rx="2" fill="rgba(99,102,241,0.2)"/>
        <!-- SCREEN -->
        <rect x="355" y="268" width="190" height="106" rx="10" fill="url(#lp-sg)" stroke="#06b6d4" stroke-width="1.5" opacity="0.92"/>
        ...
        <rect x="360" y="413" width="162" height="3" rx="1.5" fill="rgba(99,102,241,0.42)" filter="url(#lp-gv)"/>
        <!-- SHOULDERS -->
        ...
        <rect x="240" y="244" width="72" height="34" rx="12" fill="url(#lp-alg)" stroke="#4f46e5" stroke-width="1.5"/>
        <circle cx="252" cy="261" r="13" fill="#1e1b4b" stroke="#6366f1" stroke-width="1.5"/><circle cx="252" cy="261" r="7" fill="#312e81" stroke="#818cf8" stroke-width="1"/>
        <rect x="178" y="246" width="78" height="28" rx="10" fill="url(#lp-alg)" stroke="#4f46e5" stroke-width="1.5"/>
        ...
        <!-- SPRAY CAN -->
        <rect x="90" y="82" width="34" height="80" rx="10" fill="url(#lp-spg)" stroke="#c026d3" stroke-width="1.5" filter="url(#lp-gp)"/>
        ...
        <g filter="url(#lp-gp)" opacity="0.88">
        <ellipse cx="93" cy="54" rx="28" ry="20" fill="url(#lp-spb)" opacity="0.72"/><ellipse cx="76" cy="40" rx="18" ry="13" fill="rgba(192,38,211,0.5)"/><ellipse cx="113" cy="42" rx="15" ry="10" fill="rgba(124,58,237,0.5)"/>
        <circle cx="58" cy="34" r="3.5" fill="#f0abfc" opacity="0.7"/><circle cx="70" cy="24" r="2.5" fill="#e879f9" opacity="0.6"/><circle cx="83" cy="18" r="2" fill="#c026d3" opacity="0.5"/><circle cx="98" cy="20" r="3" fill="#a78bfa" opacity="0.6"/><circle cx="113" cy="26" r="2" fill="#f0abfc" opacity="0.5"/>
        </g>
        <!-- GRAFFITI TEXT -->
        <g filter="url(#lp-gp)" opacity="0.9">
        ...
        <rect x="590" y="244" width="72" height="34" rx="12" fill="url(#lp-arg)" stroke="#4f46e5" stroke-width="1.5"/>
        <circle cx="648" cy="261" r="13" fill="#1e1b4b" stroke="#6366f1" stroke-width="1.5"/><circle cx="648" cy="261" r="7" fill="#312e81" stroke="#818cf8" stroke-width="1"/>
        <rect x="644" y="246" width="78" height="28" rx="10" fill="url(#lp-arg)" stroke="#4f46e5" stroke-width="1.5"/>
        ...
        <!-- HEAD -->
        <rect x="340" y="62" width="220" height="145" rx="22" fill="url(#lp-hg)" stroke="#6366f1" stroke-width="2.5"/>
        ...
        <circle cx="384" cy="34" r="9" fill="#312e81" stroke="#818cf8" stroke-width="1.5" filter="url(#lp-gv)"/><circle cx="384" cy="34" r="5" fill="#a78bfa" filter="url(#lp-gv)"/>
        <rect x="512" y="40" width="8" height="28" rx="4" fill="#4f46e5" stroke="#818cf8" stroke-width="1"/>
        <circle cx="516" cy="34" r="9" fill="#312e81" stroke="#818cf8" stroke-width="1.5" filter="url(#lp-gv)"/><circle cx="516" cy="34" r="5" fill="#a78bfa" filter="url(#lp-gv)"/>
        <!-- HEAD BOLTS -->
        <circle cx="345" cy="102" r="6" fill="#1e1b4b" stroke="#4f46e5" stroke-width="1.5"/><circle cx="555" cy="102" r="6" fill="#1e1b4b" stroke="#4f46e5" stroke-width="1.5"/>
        <circle cx="345" cy="168" r="6" fill="#1e1b4b" stroke="#4f46e5" stroke-width="1.5"/><circle cx="555" cy="168" r="6" fill="#1e1b4b" stroke="#4f46e5" stroke-width="1.5"/>
        <!-- FACE -->
        ...
        <rect x="378" y="101" width="58" height="36" rx="8" fill="url(#lp-elg)" opacity="0.13"/>
        <circle cx="407" cy="119" r="14" fill="#0891b2" filter="url(#lp-gc)"/><circle cx="407" cy="119" r="9" fill="#06b6d4"/><circle cx="407" cy="119" r="5" fill="#a5f3fc"/>
        ...
        <rect x="464" y="101" width="58" height="36" rx="8" fill="url(#lp-erg)" opacity="0.13"/>
        <circle cx="493" cy="119" r="14" fill="#a21caf" filter="url(#lp-gp)"/><circle cx="493" cy="119" r="9" fill="#e879f9"/><circle cx="493" cy="119" r="5" fill="#f0abfc"/>
        ...
        <rect x="382" y="162" width="10" height="18" rx="3" fill="#1e1b4b"/><rect x="397" y="162" width="10" height="18" rx="3" fill="#1e1b4b"/><rect x="412" y="162" width="10" height="18" rx="3" fill="#1e1b4b"/><rect x="427" y="162" width="10" height="18" rx="3" fill="#1e1b4b"/><rect x="442" y="162" width="10" height="18" rx="3" fill="#1e1b4b"/><rect x="457" y="162" width="10" height="18" rx="3" fill="#1e1b4b"/><rect x="472" y="162" width="10" height="18" rx="3" fill="#1e1b4b"/><rect x="487" y="162" width="10" height="18" rx="3" fill="#1e1b4b"/><rect x="502" y="162" width="10" height="18" rx="3" fill="#1e1b4b"/>
        <rect x="375" y="182" width="150" height="3" rx="1.5" fill="rgba(99,102,241,0.48)" filter="url(#lp-gv)"/>
        <!-- FLOATING TAGS -->
        <g opacity="0.5" transform="rotate(12,678,120)" filter="url(#lp-gv)"><text x="618" y="137" font-family="Arial Black,sans-serif" font-size="22" font-weight="900" fill="none" stroke="#818cf8" stroke-width="2.5" stroke-linejoin="round">WRITE</text><text x="620" y="137" font-family="Arial Black,sans-serif" font-size="22" font-weight="900" fill="rgba(99,102,241,0.28)">WRITE</text></g>
        <g opacity="0.4" transform="rotate(-6,128,376)" filter="url(#lp-gc)"><text x="58" y="392" font-family="Arial Black,sans-serif" font-size="20" font-weight="900" fill="none" stroke="#06b6d4" stroke-width="2" stroke-linejoin="round">CITE</text><text x="60" y="392" font-family="Arial Black,sans-serif" font-size="20" font-weight="900" fill="rgba(6,182,212,0.22)">CITE</text></g>
        <g opacity="0.36" transform="rotate(-14,775,334)" filter="url(#lp-gp)"><text x="705" y="352" font-family="Arial Black,sans-serif" font-size="18" font-weight="900" fill="none" stroke="#e879f9" stroke-width="2" stroke-linejoin="round">THINK</text><text x="707" y="352" font-family="Arial Black,sans-serif" font-size="18" font-weight="900" fill="rgba(232,121,249,0.22)">THINK</text></g>
        <!-- PARTICLES -->
        <g filter="url(#lp-gc)" opacity="0.56"><circle cx="758" cy="170" r="3" fill="#06b6d4"/><circle cx="778" cy="200" r="2" fill="#a5f3fc"/><circle cx="748" cy="230" r="2.5" fill="#06b6d4"/></g>
        <g filter="url(#lp-gv)" opacity="0.46"><circle cx="138" cy="298" r="3" fill="#818cf8"/><circle cx="116" cy="268" r="2" fill="#c4b5fd"/><circle cx="153" cy="328" r="2.5" fill="#a78bfa"/></g>
        <g filter="url(#lp-gp)" opacity="0.36"><circle cx="818" cy="410" r="3" fill="#f0abfc"/><circle cx="838" cy="380" r="2" fill="#e879f9"/></g>
        </svg>
        </div>
        ...
        <div class="lp-title">
          ...
          <div class="lp-tag">AI &middot; Academic &middot; Writing &middot; System</div>
        </div>
        ...
        <!-- AUTH — native HTML form, no widgets, no iframe -->
        <div class="lp-hr"></div>
        <div class="lp-eyebrow">&#8212; Restricted Access &#8212;</div>
        <form class="lp-form" method="GET" action="">
          <div class="lp-iw">
            <input type="password" name="_a43" id="lp-pwd"
              placeholder="Enter access password"
              autocomplete="current-password"/>
          </div>
          <button type="submit">Enter System &nbsp;&#x2192;</button>
          <div class="lp-err">&#x26A0; Incorrect password &mdash; try again.</div>
        </form>
        <div class="lp-hr2"></div>
